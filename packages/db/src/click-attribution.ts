...
export class AffiliateLinkNotFoundError extends Error {
  ...
  constructor() {
    super("The affiliate link was not found");
...
export class AffiliateLinkStateError extends Error {
  ...
  constructor() {
    super("Only the current approved revision can create an affiliate link");
    this.name = "AffiliateLinkStateError";
  }
}
export class AffiliateLinkIdempotencyConflictError extends Error {
  readonly code = "affiliate_link_idempotency_conflict";
  constructor() {
    super("The idempotency key was used for different affiliate-link input");
    this.name = "AffiliateLinkIdempotencyConflictError";
  }
}
...
function destinationFromPayload(input: unknown): URL {
  ...
  if (typeof value !== "string" || value.length > 4_096 || /[\r\n\0]/.test(value))
    throw new AffiliateLinkStateError();
  let url: URL;
  try {
    url = new URL(value);
  } catch {
  ...
  return withTenant(context.organizationId, context.workspaceId, async (transaction) => {
    ...
    if (existing) {
      if (existing.requestFingerprint !== requestFingerprint)
        throw new AffiliateLinkIdempotencyConflictError();
      return {
        ...existing,
    ...
    const [revision] = await transaction
      ...
      .innerJoin(contentItems, eq(contentItems.id, contentRevisions.contentItemId))
      .where(
        and(
    ...
    if (
      ...
      revision.currentRevisionId !== revision.revisionId
    )
      throw new AffiliateLinkStateError();
    ...
    const [created] = await transaction
      ...
      .values({
        ...
        destinationUrlHash: createHash("sha256").update(destination.toString()).digest("hex"),
    ...
    return {
      ...created,
      contentId: input.contentId,
      revisionId: created.contentRevisionId,
      replayed: false,
    };
  });
}

export async function revokeAffiliateLink(
  context: TenantContext,
  linkId: string,
  now = new Date(),
): Promise<void> {
...
export async function resolveAffiliateLink(
  input: { organizationId: string; workspaceId: string; linkId: string; signingKeyId: string },
  now = new Date(),
): Promise<AffiliateLinkRecord> {
  const [row] = await withTenant(input.organizationId, input.workspaceId, (transaction) =>
    transaction
      .select({
        id: affiliateLinks.id,
        organizationId: affiliateLinks.organizationId,
        workspaceId: affiliateLinks.workspaceId,
        contentId: contentItems.id,
        revisionId: affiliateLinks.contentRevisionId,
        productId: affiliateLinks.productId,
        destinationUrl: affiliateLinks.destinationUrl,
        signingKeyId: affiliateLinks.signingKeyId,
        expiresAt: affiliateLinks.expiresAt,
        revokedAt: affiliateLinks.revokedAt,
      })
      .from(affiliateLinks)
      .innerJoin(contentRevisions, eq(contentRevisions.id, affiliateLinks.contentRevisionId))
      .innerJoin(contentItems, eq(contentItems.id, contentRevisions.contentItemId))
      .where(
        and(
          eq(affiliateLinks.id, input.linkId),
          eq(affiliateLinks.organizationId, input.organizationId),
          eq(affiliateLinks.workspaceId, input.workspaceId),
          eq(affiliateLinks.signingKeyId, input.signingKeyId),
        ),
      )
      .limit(1),
  );
...
export async function ingestClickEvent(
  ...
  return withTenant(envelope.organizationId, envelope.workspaceId, async (transaction) => {
    const [existing] = await transaction
      .select()
      .from(clickEvents)
      .where(eq(clickEvents.id, envelope.eventId))
      .limit(1);
    ...
    const [link] = await transaction
      ...
      .where(
        and(
          eq(affiliateLinks.id, envelope.linkId),
          eq(affiliateLinks.organizationId, envelope.organizationId),
          eq(affiliateLinks.workspaceId, envelope.workspaceId),
        ),
      )
      .limit(1);
    if (!link) throw new AffiliateLinkNotFoundError();
    await transaction.execute(
      sql`select pg_advisory_xact_lock(hashtextextended(${`${envelope.linkId}:${envelope.visitorHash}`}, 0))`,
    );
    let classification: "qualified" | "bot" | "duplicate" = envelope.botReason
      ? "bot"
      : "qualified";
    ...
    if (!envelope.botReason) {
      ...
      const [prior] = await transaction
        ...
        .where(
          and(
            eq(clickEvents.affiliateLinkId, envelope.linkId),
            eq(clickEvents.visitorHash, envelope.visitorHash),
            eq(clickEvents.classification, "qualified"),
            gte(clickEvents.receivedAt, cutoff),
          ),
        )
        .orderBy(desc(clickEvents.receivedAt))
        .limit(1);
      if (prior) {
        classification = "duplicate";
        reasonCode = "visitor_window_30m";
      }
    }
    await transaction.insert(clickEvents).values({
      id: envelope.eventId,
      organizationId: envelope.organizationId,
      workspaceId: envelope.workspaceId,
      affiliateLinkId: envelope.linkId,
      occurredAt: new Date(envelope.occurredAt),
      visitorHash: envelope.visitorHash,
      privacyKeyId: envelope.privacyKeyId,
      userAgentClass: envelope.userAgentClass,
      classification,
      reasonCode,
      requestFingerprint,
    });
