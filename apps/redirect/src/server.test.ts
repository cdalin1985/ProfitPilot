...
describe("redirect boundary", () => {
  it("fails closed without disclosing why a token is invalid", async () => {
    ...
    const server = await buildRedirectServer({ signingKeys: { test: key }, privacyKey: key, privacyKeyId: "test", eventAuthKey: key, ingestionUrl: "https://events.example.test/internal/v1/click-events" });
