...
function visitorHash(key: string, ip: string, userAgent: string): string {
  ...
  return createHmac("sha256", key)
    .update(`${prefix}\0${userAgent.slice(0, 256)}`)
    .digest("hex");
}
  ...
  server.route<{ Params: { token: string } }>({
    ...
    handler: async (request, reply) => {
      try {
        ...
        const envelope = clickEventEnvelopeSchema.parse({
          eventId: randomUUID(),
          linkId: link.id,
          ...
          privacyKeyId: config.privacyKeyId,
          userAgentClass: traffic.userAgentClass,
          botReason: traffic.botReason,
        });
        const body = JSON.stringify(envelope);
        const timestamp = String(Math.floor(Date.now() / 1000));
        const signature = createHmac("sha256", config.eventAuthKey)
          .update(`${timestamp}.${body}`)
          .digest("base64url");
        try {
          await fetch(config.ingestionUrl, {
            method: "POST",
            headers: {
          ...
          /* analytics must not block a valid navigation */
        }
        return reply
          .headers({
          ...
          .redirect(link.destinationUrl, 302);
      } catch (error) {
        const status =
          error instanceof InvalidClickTokenError
            ? 404
            : error instanceof AffiliateLinkNotFoundError
              ? 410
              : 404;
        return reply
          .status(status)
